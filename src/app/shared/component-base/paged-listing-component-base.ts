 ...
 * 分页结果dto
 */
export class PagedRequestDto<QueryData> {
  // skipCount: number;
  pageSize: number;
  sorting: string;
  currentPage: number;
...
export abstract class PagedListingComponentBase<TResult, TQuery, TEdit>
  implements OnInit {

  /**
  * 查询DTO
  */
  public queryDto: TQuery;
  /**
 * 编辑DTO
 */
  public editDto: TEdit;
 ...
 * SF服务器端分页参数
 */
  public pager = { front: false, show: true };
  /**
   * 分页大小
   */
 ...
 */
  ...
  public getDataPage(page: number): void {
    ...
    //   return;
    // }
    const req = new PagedRequestDto<TQuery>();
    req.pageSize = this.pageSize;
    req.currentPage = page;
    req.sorting = this.sorting;
    req.searchKey = this.searchKey;
    req.data = this.queryDto;
  ...
  protected abstract fetchDataList(
    request: PagedRequestDto<TQuery>,
    pageNumber: number,
